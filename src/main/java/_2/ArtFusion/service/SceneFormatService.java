...
public class SceneFormatService {
    ...
    public _2.ArtFusion.domain.storyboard.StoryBoard getSceneFormatData(Long userId, Long storyId) {
        ...
        _2.ArtFusion.domain.storyboard.StoryBoard storyBoard = storyBoardRepository.findById(storyId).orElseThrow();

        List<_2.ArtFusion.domain.scene.SceneFormat> scenes = sceneFormatRepository.findScenesByStoryBoard(storyBoard);
        for (_2.ArtFusion.domain.scene.SceneFormat scene : scenes) {
